...
exports.isValidTurn = function (board, player, fromId, toId) {
	...
	if(typeof toId === 'undefined' || toId === null) { return false; }

	// players are not allowed to place tokens on already occupied fields
	if(! board.field[toId].isEmpty() ) { return false; }
	...
	switch (player.phase) {
		...
		case GAME_CONST.GAME_PHASE_1:
			...
			if(typeof fromId !== 'undefined' && fromId !== null){ return false; }

			// players must have tokens left in their hand
			if(player.noTokensInHand <= 0) { return false; }
		...
		case GAME_CONST.GAME_PHASE_2:
			...
			if(board.field[fromId].isEmpty() ) { return false; }

			// players may only move their own tokens
			if( board.field[fromId].token !== player.token ) { return false; }

			// "from" and "to" need to be adjacent
			if(! fieldGraph.neighbors(fromId, toId) ) { return false; }
...
exports.willCloseMill = function (board, player, fromId, toId) {
	...
	const isMill = (row) => {
		...
		return row.filter(r => {
			...
			if(r === fromId) { return false; }
			// assume "to" to always contain a player token
			if(r === toId) { return true; }
